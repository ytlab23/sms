...
// //           const data = pageSnapshot.data()
// //           const content = {
// //             ...data.pageContent?.[lang as keyof typeof data.pageContent],
// //             service: data.service || ''
// //           }
// //           setPageData(content)
...
// //         description: 'You Can Now Use The Service',
// //       })
// //       navigate(`/${lang}/${t("urls.sms")}?id=${id}`)
...
// //             action: (
// //               <ToastAction
// //                 onClick={() => navigate(`/${lang}/${t("urls.pay")}`)}
// //                 altText='Go To Payment'
// //               >
// //                 Go To Payment
// //               </ToastAction>
...
//         // Query for pages with matching slugs in any language
//         const pagesQuery = query(
//           collection(db, 'internal_pages'),
//           where(`pageContent.${lang}.slug`, '==', slug)
//         )
//         const pagesSnapshot = await getDocs(pagesQuery)

//         let pageDoc = pagesSnapshot.docs[0]

//         // If not found, try searching with English slug
//         if (!pageDoc && lang !== 'en') {
//           const englishQuery = query(
//             collection(db, 'internal_pages'),
//             where('pageContent.en.slug', '==', slug)
//           )
//           const englishSnapshot = await getDocs(englishQuery)
//           pageDoc = englishSnapshot.docs[0]
//         }

//         if (pageDoc) {
//           const data = pageDoc.data()
//           const content = {
//             // ...data.pageContent?.[lang],
//             ...data.pageContent?.[lang as keyof typeof data.pageContent],
//             service: data.service || ''
//           }
//           setPageData(content)
...
export default function InternalPage() {
  const { lang, slug: rawSlug } = useParams<{ lang: string; slug: string }>()
  const { t, i18n } = useTranslation()
  const navigate = useNavigate()
  const { toast } = useToast()
  const [pageData, setPageData] = useState<PageData | null>(null)
  ...
  useEffect(() => {
    const cleanSlug = (rawSlug: string) => {
      return rawSlug.startsWith('urls.reverseUrls.') ? rawSlug.replace('urls.reverseUrls.', '') : rawSlug
    }
    ...
    const fetchPageData = async () => {
      try {
        ...
        // Query for pages with matching slugs in any language
        const languages = ['en', 'es', 'fr', 'de', 'it', 'pt', 'ru', 'ja', 'ko', 'zh'] // Add all supported languages
        const queries = languages.map(lang => 
          query(collection(db, 'internal_pages'), where(`pageContent.${lang}.slug`, '==', slug))
        )

        const querySnapshots = await Promise.all(queries.map(q => getDocs(q)))
        const allDocs = querySnapshots.flatMap(snapshot => snapshot.docs)

        let pageDoc = allDocs[0]

        if (pageDoc) {
          const data = pageDoc.data()
          const content = {
            // ...data.pageContent?.[lang] || data.pageContent?.en, // Fallback to English if current language is not available
            ...data.pageContent?.[lang as keyof typeof data.pageContent] || data.pageContent?.en, // Fallback to English if current language is not available
