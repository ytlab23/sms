...
// export function ChooseService({isSide = true}) {
...
//   return (
//     <div className="flex flex-col   mx-auto p-4 space-y-4 bg-background rounded-lg shadow">
...
//         </Button>
//       </div>
//     <div className={isSide? "": "flex flex-col md:flex-row"}>
//       {serviceFirst ? (
//         <>
//           <ServiceSelector      
//              order="1"
...
//       <Button
//         onClick={buyProduct}
//         className={isSide ? "w-full  text-white":"w-auto px-4 py-2 text-white bg-blue-500 hover:bg-blue-600 rounded-lg"} 
//         disabled={!selectedCountry || !selectedService || buying}
//       >
//         {buying ? (
//           <RefreshCcw className="mr-2 h-4 w-4 animate-spin" />
...
import { db } from "./../../firebase/config"
import axios from "axios"
import type { Country, Service } from "./buy-service/types"
...
export function ChooseService({ isSide = true }) {
  ...
  const [statistics, setStatistics] = useState<{
    [key: string]: { successfulRefunds: number; unsuccessfulRefunds: number }
  }>({})
  ...
  const getSuccessRate = (countryName: string, serviceName: string) => {
    ...
    const storedSuccessRate = localStorage.getItem(key)
    if (storedSuccessRate) {
      return Number.parseFloat(storedSuccessRate) // Return the stored value
    }
  ...
  return (
    <div className="flex flex-col mx-auto p-4 space-y-4 bg-background rounded-lg shadow">
      ...
      <div className={isSide ? "" : "flex flex-col md:flex-row md:space-x-4"}>
        {serviceFirst ? (
          <>
            <div className={isSide ? "" : "w-full md:w-1/2"}>
              <ServiceSelector
                order="1"
                selectedService={selectedService}
                setSelectedService={setSelectedService}
                ...
                getServicePrice={getServicePrice}
                allPricingData={allPricingData}
                getSuccessRate={(serviceName) =>
                  selectedCountry ? getSuccessRate(selectedCountry.name, serviceName) : null
                }
              />
            </div>
            <div className={isSide ? "" : "w-full md:w-1/2"}>
              <CountrySelector
                ...
                favorites={favorites}
                toggleFavorite={toggleFavorite}
                getSuccessRate={(countryName) =>
                  selectedService ? getSuccessRate(countryName, selectedService.name) : null
                }
              />
            </div>
          </>
      ...
      <div className="flex justify-center ">
        <Button
          onClick={buyProduct}
          className={
            isSide ? "w-full text-white" : "w-auto px-4 py-2 text-white bg-blue-500 hover:bg-blue-600 rounded-lg"
          }
          disabled={!selectedCountry || !selectedService || buying}
        >
          {buying ? <RefreshCcw className="mr-2 h-4 w-4 animate-spin" /> : <ShoppingCart className="mr-2 h-4 w-4" />}
          {t("actionsidebar.Buy number")}
        </Button>
      </div>
    </div>
